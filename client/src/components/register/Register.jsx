...
const Register = () => {
	...
	const validateData = (data, e) => {
		...
		if (!data.name) {
			enqueueSnackbar('username is requires', {
				variant: 'warning',
				anchorOrigin: {
					vertical: 'bottom',
					horizontal: 'center',
	...
	const registerUser = async (formData) => {
		await axios
			.post(config.endPoint + '/api/register', formData)
			.then(() => {
	...
	return (
		<>
			...
			<div className='w-full h-[80vh]'>
				<form
					onSubmit={(e) => {
						if (validateData(formData, e)) {
							registerUser(formData);
						}
					}}
					className='m-auto flex flex-col justify-center items-center h-full'>
					<h1 className='mb-10 text-2xl uppercase'>Register</h1>
